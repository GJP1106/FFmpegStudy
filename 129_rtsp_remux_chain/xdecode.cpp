...
bool XDecode::Recv(AVFrame * frame, bool is_hw_type)
{
	unique_lock<mutex> lock(mux_);
	if (!c_) return false;
	auto f = frame;
	if (is_hw_type && c_->hw_device_ctx) {  //硬件加速
		f = av_frame_alloc();
	}
	auto re = avcodec_receive_frame(c_, f);
	if (re == 0) {
		if (is_hw_type && c_->hw_device_ctx) {  //GPU 解码
			// 显存转内存 GPU-》CPU
	...
	if (is_hw_type && c_->hw_device_ctx) {
		av_frame_free(&f);
	}
	return false;
}
